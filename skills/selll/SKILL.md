...
description: >
  ...
  books and closes deals on autopilot. Built by Aaron Shepard, SELLL.
  ...
  Triggers on: "revenue engine", "SELLL", "outbound engine", "build the
...
> — Aaron Shepard, Founder, SELLL

---

## What This Is

The SELLL Revenue Engine is a six-layer B2B outbound system that runs continuously, 24/7, to find the right prospects at the right moment, engage them with the right message, book meetings automatically, and close deals without requiring a full sales team.
...
Every time you run the SELLL engine:
...
| 2 | `SELLL/lead-scoring` | Score all accounts from behavioral + firmographic + timing signals |
| 3 | `SELLL/meeting-automation` | For HOT leads: draft booking email. Confirm meeting. Trigger account research. |
...
| Step | Skill | What It Does |
|------|-------|-------------|
| 1 | `SELLL/deal-nurture` | Build and run post-meeting sequence: summary → value → case study → proposal → close |
...
| Step | Skill | What It Does |
|------|-------|-------------|
| 1 | `SELLL/revenue-reporting` | Weekly report: campaign metrics, pipeline health, meetings, deals, forecast |
| 2 | `context-building` (feedback mode) | Import campaign results. Promote validated hypotheses. Retire dead angles. |
| 3 | `SELLL/signal-monitor` | Scan for new buying signals. Flag new accounts for activation. |
...
└── references/
    ...
    ├── kpi-benchmarks.md            ← SELLL targets vs. industry averages
```
...
After the build, every list, template, campaign, and system belongs to the client. No vendor lock-in. No dependency on SELLL to run.
...
*SELLL Revenue Engine v2.0 — Built to compound. Designed to run without you.*
